...
public class AutoTest2 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectorySequence trajSeq2 = drive.trajectorySequenceBuilder(trajSeq.end())
                ...
                .lineToLinearHeading(new Pose2d(startPose.getX() + -56, startPose.getY() -70, Math.toRadians(-69)))
                .build();
        ...
        while(cycle <= 3){
            ...
            if(search){
                if(camCycle % 15 == 0){
                    ...
                    else if(Math.abs(lastX - currentX) > 1 && move && lastX != 0){
                        power -= 0.01;
                    }
                    else{
                        count = 0;
                ...
                currentX = pipeline.getXContour();
                if(currentX > target - 50 && currentX < target + 50){
                    move = false;
                    DriveTrain.turn(0);
                    lastX = pipeline.getXContour();
                    count = 0;
                }
                else if(currentX < target && lastX != 0){
